	...
	class productController extends mainModel{
		...
		public function registrarProductoControlador(){
    		...
    		if($_FILES['producto_foto']['name']!="" && $_FILES['producto_foto']['size']>0){
				...
				//antes de los cambios punto de referencia para el control + z
		        # Nombre de la foto #
			...
			if (isset($busqueda) && $busqueda != "") {
				$consulta_datos = "SELECT $campos 
					...
					WHERE producto_codigo LIKE '%$busqueda%' 
						...
						OR producto_marca LIKE '%$busqueda%' 
						OR producto_modelo LIKE '%$busqueda%' 
			...
			try {
				...
				if ($total >= 1 && $pagina <= $numeroPaginas) {
					...
					foreach ($datos as $rows) {
						$fotoProducto = is_file("./app/views/productos/" . $rows['producto_foto']) 
							? APP_URL . "app/views/productos/" . $rows['producto_foto'] 
							...
							<div class="media-content">
								<div class="content">
									<p>
										...
										<strong>PRECIO:</strong> Bs.' . $rows['producto_precio_venta'] . ', 
					...
					$tabla .= $total >= 1 
						? '<p class="has-text-centered"> <i class="far fa-hand-point-down fa-5x"></i></p>
						   <p class="has-text-centered"><a href="' . $url . '1/" class="button is-link is-rounded is-small">Recargar listado</a></p>'
						: '<p class="has-text-centered"> <i class="far fa-grin-beam-sweat fa-5x"></i></p>
				...
				$tabla = '<p class="has-text-centered">Ocurrió un error al obtener los datos: ' . $e->getMessage() . '</p>';
			}
		...
		public function obtenerProductos($pagina = 1, $productosPorPagina = 12) {
			$offset = ($pagina - 1) * $productosPorPagina;
	
			$query = "SELECT * FROM producto LIMIT :offset, :productosPorPagina";
		...
		public function contarProductos() {
			...
			$stmt = $this->conectar()->query($query);
